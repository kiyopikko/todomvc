(function (window) {
	 ...
	 */
	function Controller(model, view, storage) {
		var that = this;
		that.model = model;
		that.view = view;
		that.storage = storage;

		var todosDS = that.storage.dataStore.child("todos");

		todosDS.on('push', function(data){
			that._filter(true);
		});
		todosDS.on('remove', function(data){
			that._filter(true);
		});
		todosDS.on('set', function(data){
			that._filter(true);
		});
	...
	Controller.prototype.editItemSave = function (id, title) {
		...
		if (title.trim()) {
			that.model.update(id, {title: title}, function () {
				...
				that.todosDS.on('set', function(data){
	...
	Controller.prototype._filter = function (force) {
		...
		// Real Time ver.
		if (force || this._lastActiveRoute !== activeRoute) {
			this['show' + activeRoute]();
		}
